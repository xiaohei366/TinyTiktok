...
import (
	...
	"github.com/xiaohei366/TinyTiktok/cmd/user/initialize/mq/amqpclt"
...
func InitMq() {
	...
	AddActor, err = amqpclt.NewActor(amqpConn, "cnt_add")
	if err != nil {
		klog.Fatal("cannot create add actor", err)
	}
	//开启消费监听
	go AddActor.Consumer(context.Background())
	DelActor, err = amqpclt.NewActor(amqpConn, "cnt_del")
	if err != nil {
